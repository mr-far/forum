...
async fn register(
    ...
    let _ = app.database.create_secret(id, payload.password.clone())
        .await.map_err(|err| HttpError::Database(err))?;
    let token = user.secret(app.get_ref()).await.token().expose_secret().to_owned();
